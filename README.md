...
1. **DDL Auto Mode**
   - 현재: `create` (애플리케이션 재시작 시 테이블 삭제/재생성)
   - 권장: 프로덕션 환경에서 `update` 또는 `validate` 사용

2. **CORS 설정**
   - 현재: 모든 오리진 허용
   - 권장: 프로덕션 환경에서 특정 도메인만 허용

3. **데이터베이스 인증정보**
   - 현재: application.yml에 평문 저장
   - 권장: 환경 변수 사용 (`${DB_PASSWORD}`)

4. **포트 설정**
   - MySQL 비표준 포트 사용 (3302)
   - 필요시 표준 포트(3306)로 변경 고려
